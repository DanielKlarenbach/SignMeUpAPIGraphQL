...
from api.models import FieldOfStudy, Subject, Student, Points, SubjectGroup, SubjectType
...
class MakeGroups(graphene.Mutation):
    ...
    def mutate(cls, root, info, field_of_study_id):
        subjects_types = SubjectType.objects.filter(field_of_study_id=field_of_study_id
                                                    )
        for subject_type in subjects_types:
            subjects_without_free_places = []
            students_with_subject_group = []

            points_by_subject_type = Points.objects.filter(subject__subject_type=subject_type)
            ...
            while current_max != 0:
                ...
                for points in points_by_current_max:
                    ...
                    subject_group = SubjectGroup.objects.filter(subject__subject_type=subject_type, student=student)
            ...
            subjects_with_free_places = Subject.objects.filter(subject_type=subject_type, type='P').filter(
            ...
            for student in students_without_subject_group:
                for subject_with_free_places in subjects_with_free_places:
                    ...
                    subject_with_free_places.occupancy+=1
                    subject_with_free_places.save()
