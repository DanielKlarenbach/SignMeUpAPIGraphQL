...
class UploadFile(graphene.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, root, info, **input):
        ...
        for i in range(0, len(df)):
            try:
                field_of_study = FieldOfStudy.objects.get(name=df['field_of_study'][i], year__department=department,
                                                          year__start_year=df['year'][i])
            except FieldOfStudy.DoesNotExist:
                raise Exception(
                    "This field of study doesn't exist " + str(df['field_of_study'][i]) + str(df['year'][i]))
            try:
